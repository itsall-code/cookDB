...
> 重要：本工具会执行清库、删除、覆盖导入、DDL/DML 等不可逆操作。
...
用途：把一个或多个 MySQL 连接的 database 导出为 SQL 文件，保存在 `backend/backup/` 目录。
...
1. 在「批量导出」区域勾选多个分支，也可使用「全选」。
2. 点击「开始批量导出」并确认目标列表。
3. 工作台按顺序逐个导出，进度区展示当前任务。
4. 点击「取消」会取消当前任务并停止剩余队列。

说明：

- 文件名格式为 `{database}_{含毫秒时间戳}.sql`，避免批量任务重名覆盖。
